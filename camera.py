...
class Camera:
    ...
    def update(self):
        ...
        if (delta.x > self.center.x * self.delta_threshold):
            self.real_pos = util.vec2(self.real_pos.x + 1.0, self.real_pos.y)#util.vec2(self.lookat_pos.x + 1.0, self.lookat_pos.y)
        elif (-delta.x > self.center.x * self.delta_threshold):
            self.real_pos = util.vec2(self.real_pos.x - 1.0, self.real_pos.y)
